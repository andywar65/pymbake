...
The app can be cloned or downloaded from [Github](https://github.com/andywar65/pymbake). Using a shell get into the project folder and type  `git clone https://github.com/andywar65/pymbake`. Add `pymba` to the INSTALLED_APPS in your settings file. Migrate and Collectstatic. The app's templates look for a `base.html` file, so be sure to have one.
